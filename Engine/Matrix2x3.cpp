...
Point2f Matrix2x3::ExtractTranslate() const
{
	return Point2f{ orig.x, orig.y };
}

float Matrix2x3::ExtractRotate() const
{
	return atan2(dirX.y, dirY.x);
}

float Matrix2x3::ExtractScaleX() const
{
	const float scale{ sqrt(powf(dirX.x, 2) + powf(dirY.x, 2)) };
	return (signbit(dirX.x)) ? -scale : scale;
}

float Matrix2x3::ExtractScaleY() const
{
	const float scale{ sqrt(powf(dirX.y, 2) + powf(dirY.y, 2)) };
	return (signbit(dirX.x)) ? -scale : scale;
}
